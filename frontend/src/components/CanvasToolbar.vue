<template>
  <v-card class="toolbar-card rounded-pill px-4 py-2 d-flex align-center gap-4" elevation="4">
    <!-- Tool Selector -->
    <div class="d-flex align-center gap-1">
      <v-btn icon size="small" :variant="modelValue.tool === 'brush' ? 'flat' : 'text'" :color="modelValue.tool === 'brush' ? 'primary' : 'default'" @click="updateSettings('tool', 'brush')">
        <v-tooltip activator="parent" location="top">Pincel</v-tooltip>
        <v-icon>mdi-brush</v-icon>
      </v-btn>
      <v-btn icon size="small" :variant="modelValue.tool === 'marker' ? 'flat' : 'text'" :color="modelValue.tool === 'marker' ? 'primary' : 'default'" @click="updateSettings('tool', 'marker')">
        <v-tooltip activator="parent" location="top">Rotulador</v-tooltip>
        <v-icon>mdi-marker</v-icon>
      </v-btn>
      <v-btn icon size="small" :variant="modelValue.tool === 'fill' ? 'flat' : 'text'" :color="modelValue.tool === 'fill' ? 'primary' : 'default'" @click="updateSettings('tool', 'fill')">
        <v-tooltip activator="parent" location="top">Rellenar</v-tooltip>
        <v-icon>mdi-format-color-fill</v-icon>
      </v-btn>
      <v-btn icon size="small" :variant="modelValue.tool === 'eraser' ? 'flat' : 'text'" :color="modelValue.tool === 'eraser' ? 'primary' : 'default'" @click="updateSettings('tool', 'eraser')">
    ...
    <div class="d-flex align-center gap-2" :style="{ opacity: modelValue.tool === 'eraser' ? 0.5 : 1, pointerEvents: modelValue.tool === 'eraser' ? 'none' : 'auto' }">
      <v-btn
      ...
      >
        <v-icon v-if="modelValue.color === c.value" size="16" color="white" style="mix-blend-mode: difference;">
          mdi-check
        </v-icon>
      </v-btn>
    </div>
    ...
    <div class="d-flex align-center gap-1">
      ...
      >
        <v-icon size="24">mdi-circle</v-icon>
      </v-btn>
    </div>
    ...
    <v-sheet elevation="0" class="d-flex align-center bg-transparent gap-1">
      <v-btn icon size="small" variant="text" :disabled="!canUndo" @click="$emit('undo')">
        <v-tooltip activator="parent" location="top">Deshacer</v-tooltip>
        <v-icon>mdi-undo</v-icon>
      </v-btn>

      <v-btn icon size="small" variant="text" :disabled="!canRedo" @click="$emit('redo')">
      ...
      <!-- Magic Eraser -->
      <v-btn icon size="small" variant="text" color="info" @click="$emit('clear-sketch')">
        <v-tooltip activator="parent" location="top">Varita Mágica (Borra Boceto Azul)</v-tooltip>
        <v-icon>mdi-magic-staff</v-icon>
      </v-btn>

      <!-- Clear Canvas -->
      <v-btn icon size="small" variant="text" color="error" @click="$emit('clear')">
      ...
      <!-- Universal Save -->
      <v-btn icon size="small" variant="text" color="success" @click="$emit('save')">
